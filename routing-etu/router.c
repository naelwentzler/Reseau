...
int forward_packet(packet_data_t *packet, int psize, routing_table_t *rt) {
    ...
    int sock_id;
    unsigned char packet_Dest= packet->dst_id;

    for(int i=0; i < rt->size; i++){
        node_id_t prochain_Dest=rt->tab[i].dest;

        if(packet_Dest==prochain_Dest){
    ...
    server_adr.sin_port = rt->tab[i].nexthop.port;
    server_adr.sin_addr.s_addr = rt->tab[i].nexthop.ipv4;
            ...
            case DATA:
                ...
                if (pdata->dst_id == MY_ID) {
                    ...
                    if (pdata->ttl==0){
                        ...
                        forward_packet(&pdata, sizeof(packet_data_t), rt);
                    }
